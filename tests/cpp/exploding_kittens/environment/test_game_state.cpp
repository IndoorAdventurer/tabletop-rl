...
TEST(GameStateTests, RegisterTurnTest) {
    GameState g;
    g.reset(2);
    EXPECT_EQ(g.primary_player, 0) << "Player 0 should be first to get turn.";
    g.register_turn();
    EXPECT_EQ(g.primary_player, 1) << "After player 0 comes player 1 ofc.";
    g.register_turn();
    EXPECT_EQ(g.primary_player, 0) << "And then we loop back around to 0";
    ...
    for (uint8_t i : to_expect) {
        g.register_turn();
        EXPECT_EQ(g.primary_player, i) << "Expect players getting turn in order.";
    }
    ...
    g.register_turn();
    EXPECT_EQ(g.primary_player, 0) << "Should still be 0 because 3 turns.";
    ...
    g.register_turn();
    EXPECT_EQ(g.primary_player, 1) << "Now 3 turns taken, so next player is.";
    EXPECT_EQ(g.turns_left, 1) << "One turns should be left";
}
...
TEST(GameStateTests, RegisterTurnWithDeadPlayersTest) {
    ...
    for (uint8_t i : to_expect) {
        g.register_turn();
        EXPECT_EQ(g.primary_player, i) << "Player 1 is dead and should be skipped.";
    }

    while (g.is_alive(2))
        g.cards.hands[2].take_from(g.cards.deck);
    
    EXPECT_EQ(g.primary_player, 0) << "We left off at player 0";
    uint8_t to_expect2[] = {3, 4, 0, 3, 4, 0};
    for (uint8_t i : to_expect2) {
        g.register_turn();
        EXPECT_EQ(g.primary_player, i) << "Player 2 now also skipped.";
    }
    ...
    for (uint8_t i : to_expect3) {
        g.register_turn();
        EXPECT_EQ(g.primary_player, i) << "Only 0 and 3 left.";
    }
    ...
    for (uint8_t i = 0; i != 10; ++i) {
        g.register_turn();
        EXPECT_EQ(g.primary_player, 3) << "Only 3 is left so should go here.";
    }
